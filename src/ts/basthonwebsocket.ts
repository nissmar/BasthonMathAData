...
class DomNodeBus {
    ...
    public push(obj: any): number {
        let id = 0;
        for (; id < this._bus.size; id++)
            if (!(this._bus.has(id))) break;
...
class EvalQueue {
     ...
     */
    public pop(): any { return this._queue.shift(); }
    ...
    public popAndRun(): any {
        ...
        if (data) {
            ...
            this.ws._send(this.ws._format_msg(
...
export class BasthonWebSocket {
    ...
    private _connectEvents(kernel: KernelBase) {
        ...
        this.kernel.addEventListener('eval.output', (data: any) => {
            this._send(this._format_msg(data.parent_msg, "stream", {
                name: data.stream,
                text: data.content
            }, "iopub"));
        });
        ...
        this.kernel.addEventListener('eval.display', (data) => {
            ...
            switch (data.display_type) {
                ...
                case "p5":
                    ...
                    */
                    ...
                    send_data = { "application/javascript": `element.append(window._basthonDomNodeBus.pop(${id}));` };
                    break;
                case "multiple":
            ...
            this._send(this._format_msg(data.parent_msg, "display_data", {
                data: send_data,
                metadata: {},
                transcient: {},
            }, "iopub"));
    ...
    public close() { this.onclose?.call(this); }
}
