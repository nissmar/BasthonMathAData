...
define([
  ...
  Notebook.prototype.bind_events = function () {
    ...
    this.events.on("spec_changed.Kernel", function (event, data) {
      ...
      if (
        !existing_spec ||
        !_.isEqual(existing_spec, that.metadata.kernelspec)
      ) {
        that.set_dirty(true);
      }
      // start a new session
  ...
  Notebook.prototype.get_selected_cells = function () {
    return this.get_cells().filter(function (cell, index) {
      return cell.selected || soft_selected(cell) || cell.anchor;
    });
  };
  ...
  Notebook.prototype.to_code = function (index) {
    ...
    if (this.is_valid_cell_index(i)) {
      ...
      if (
        !(source_cell instanceof codecell.CodeCell) &&
        source_cell.is_editable()
      ) {
  ...
  Notebook.prototype.to_markdown = function (index) {
    ...
    if (this.is_valid_cell_index(i)) {
      ...
      if (
        !(source_cell instanceof textcell.MarkdownCell) &&
        source_cell.is_editable()
      ) {
  ...
  Notebook.prototype.to_raw = function (index) {
    ...
    if (this.is_valid_cell_index(i)) {
      ...
      if (
        !(source_cell instanceof textcell.RawCell) &&
        source_cell.is_editable()
      ) {
  ...
  Notebook.prototype._warn_heading = function () {
    dialog.modal({
      ...
      body: $("<p/>")
        ...
        .append($("<pre/>").text(i18n.msg._("## This is a level 2 heading"))),
      buttons: {
        OK: {},
      },
    });
  ...
  Notebook.prototype.paste_cell_replace = function () {
    ...
    for (var i = this.clipboard.length - 1; i >= 0; i--) {
      ...
      var new_cell = this.insert_cell_at_index(
        cell_data.cell_type,
        insertion_index
      );
      new_cell.fromJSON(cell_data);
  ...
  Notebook.prototype.set_theme = function (theme) {
    ...
    if (cm_config != null)
      cm_config.theme = theme === "ipython" ? undefined : theme;
  ...
  Notebook.prototype.paste_cell_attachments = function () {
    if (this.clipboard_attachments !== null && this.paste_attachments_enabled) {
      var cell = this.get_selected_cell();
      if (cell.attachments === undefined) {
        cell.attachments = {};
      }
  ...
  Notebook.prototype.cell_toggle_line_numbers = function () {
    this.get_selected_cells().map(function (cell) {
  ...
  Notebook.prototype._restart_kernel = function (options) {
    ...
    if (options.confirm === false || !that.kernel.is_connected()) {
      var default_button =
        options.dialog.buttons[Object.keys(options.dialog.buttons)[0]];
    ...
    if (check_last_modified) {
      return this.contents.get(this.notebook_path, { content: false }).then(
        function (data) {
          ...
          var last_modified_check_margin =
            (that.config.data["last_modified_check_margin"] || 0.5) * 1000; // 500 ms
          ...
          if (
            last_modified.getTime() - that.last_modified.getTime() >
            ...
            if (that._changed_on_disk_dialog !== null) {
              ...
              that._changed_on_disk_dialog = dialog.modal({
                ...
                body: i18n.msg._(
                  "The notebook file has changed on disk since the last time we opened or saved it. " +
        ...
        Save: {
          ...
          click: function () {
            ...
            var save_thunk = function () {
              ...
              return that.contents.save(nb_path, model).then(
                function (data) {
                  d.modal("hide");
                ...
                function (error) {
                  var msg = i18n.msg._(
                    error.message || "Unknown error saving notebook"
                  );
                  $(".save-message").html(
                    $("<span>").attr("style", "color:red;").text(msg)
                  );
                }
              );
            };
            that.contents
              .get(nb_path, { type: "notebook", content: false })
              .then(
                function (data) {
                  var warning_body = $("<div/>").append(
                    $("<p/>").text(
                      i18n.msg._("Notebook with that name exists.")
                    )
                  );
                  dialog.modal({
                    title: "Save As",
        ...
        Trust: {
          ...
          click: function () {
            ...
            return pr.then(function () {
              nb.contents.trust(nb.notebook_path).then(
                function () {
  ...
  Notebook.prototype.rename = function (new_name) {
    ...
    return this.contents
      .rename(this.notebook_path, new_path)
      .then(function (json) {
  ...
  Notebook.prototype.load_notebook = function (notebook_path) {
    ...
    this.contents
      .get(notebook_path, { type: "notebook" })
      .then(
  ...
  Notebook.prototype.load_notebook_success = function (data) {
    ...
    if (failed || data.message) {
      ...
      if (failed) {
        ...
        body
          .append($("<p>").text(i18n.msg._("The error was: ")))
          .append($("<div>").addClass("js-error").text(failed.toString()))
          .append(
            $("<p>").text(i18n.msg._("See the error console for details."))
          );
      } else {
      ...
      if (data.message) {
        if (failed) {
          ...
          msg = i18n.msg._(
            "An invalid notebook may not function properly." +
              " The validation error was:"
          );
        }
        body
    ...
    if (orig_nbformat !== undefined && nbmodel.nbformat !== orig_nbformat) {
      var oldmsg = i18n.msg._(
        "This notebook has been converted from an older notebook format" +
          " to the current notebook format v(%s)."
      );
      var newmsg = i18n.msg._(
        "This notebook has been converted from a newer notebook format" +
          " to the current notebook format v(%s)."
      );
  ...
  Notebook.prototype.load_notebook_error = function (error) {
    ...
    if (navigator.cookieEnabled == false) {
      msg = i18n.msg._(
        "Jupyter requires cookies to work; please enable cookies" +
  ...
  Notebook.prototype.list_checkpoints = function () {
    ...
    this.contents
      ...
      .then($.proxy(this.list_checkpoints_success, this), function (error) {
  ...
  Notebook.prototype.create_checkpoint = function () {
    ...
    this.contents
      ...
      .then($.proxy(this.create_checkpoint_success, this), function (error) {
  ...
  Notebook.prototype.restore_checkpoint_dialog = function (checkpoint) {
    ...
    var body = $("<div/>")
      ...
      .append(
        $("<p/>")
          .addClass("p-space")
          .text(i18n.msg._("The checkpoint was last updated at:"))
      )
      .append(
        $("<p/>")
          .addClass("p-space")
          .text(
            moment(checkpoint.last_modified).format("LLLL") +
              " (" +
              moment(checkpoint.last_modified).fromNow() +
              ")" // Long form:  Tuesday, January 27, 2015 12:15 PM
          )
  ...
  Notebook.prototype.restore_checkpoint = function (checkpoint) {
    ...
    var that = this;
    this.contents
      .restore_checkpoint(this.notebook_path, checkpoint)
      .then($.proxy(this.restore_checkpoint_success, this), function (error) {
  ...
  Notebook.prototype.delete_checkpoint = function (checkpoint) {
    ...
    var that = this;
    this.contents
      .delete_checkpoint(this.notebook_path, checkpoint)
      .then($.proxy(this.delete_checkpoint_success, this), function (error) {
